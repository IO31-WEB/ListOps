...
export default function SettingsPage() {
  ...
  const securityItems = [
    {
      ...
      desc: 'Update your email address or change your password',
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {/* Account security — opens Clerk's hosted UserProfile modal */}
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <div className="flex items-center gap-2 mb-5">
        ...
        <div className="space-y-3">
          {securityItems.map(item => (
            <button
              ...
              className="w-full flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-100 hover:border-slate-200 transition-colors group text-left"
            >
              <div>
                <p className="text-sm font-semibold text-slate-900">{item.label}</p>
                <p className="text-xs text-slate-500 mt-0.5">{item.desc}</p>
              </div>
              <span className="text-xs text-amber-600 font-medium group-hover:underline flex-shrink-0">Manage →</span>
            </button>
          ))}
        </div>
      </div>
